...
<div class="min-h-screen flex items-center justify-center px-4 py-8">
    <div style="width:464px" class="mx-auto bg-white dark:bg-neutral-800 rounded-xl shadow-lg px-6 py-6">
        <div class="relative w-full">
            ...
            <h2 class="text-2xl font-bold mb-6 text-center">Edit Kartu Visitor</h2>
        </div>

        <form action="{{ route('visitor-cards.update', $card->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-5">
                ...
                >
                    <option value="1" {{ old('status', $card->status) == '1' ? 'selected' : '' }}>Aktif / Tersedia</option>
                    <option value="0" {{ old('status', $card->status) == '0' ? 'selected' : '' }}>Tidak Aktif / Dipakai</option>
                </select>
                @error('status')
